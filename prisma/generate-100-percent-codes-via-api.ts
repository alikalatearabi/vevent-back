...
function formatOutput(codes: Array<{ code: string; success: boolean }>): string {
  ...
  output += `100% Discount Codes (${successfulCodes.length} codes):\n`;
  output += '--------------------------------------------------\n';
 ...
 */
async function main() {
  console.log('Generating 50 discount codes via API...\n');
  ...
  console.log('Generating 50 unique codes...');
  const codes = generateUniqueCodes(50, existingCodes);
  ...
  const filePath = path.join(__dirname, 'discount-codes-100-percent-50-codes-api.txt');
